...
public class QuizService {
    ...
    public List<RankingDTO> getRanking() {
        ...
        // sort by percent desc, then by obtained desc, then name asc (for stable display)
        list.sort((a, b) -> {
            int c = Double.compare(b.getTotalScore(), a.getTotalScore()); // percent desc
            if (c != 0) return c;
            // If percent tie, fallback to name ascending
            return a.getName().compareToIgnoreCase(b.getName());
        });

        // assign dense ranks: equal percent => same rank; next distinct percent => rank++
        int rank = 0;
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (i == 0) {
                ...
                // if percent changed, increment rank
